...
class AssetStore:
    ...
    def initialize(self):
        with self.connect() as connection:
            ...
            columns = self.connection_manager.table_columns(
                connection,
                "assets",
            )
            ...
            if "borrowed_quantity" not in columns:
                connection.execute(
                    "ALTER TABLE assets ADD COLUMN borrowed_quantity INTEGER NOT NULL DEFAULT 0"
                )

            checkout_columns = self.connection_manager.table_columns(
                connection,
                "checkouts",
            )
            ...
            if "checkout_location" not in checkout_columns:
                connection.execute(
                    "ALTER TABLE checkouts ADD COLUMN checkout_location TEXT NOT NULL DEFAULT ''"
                )

            if "due_time" not in checkout_columns:
                connection.execute(
                    "ALTER TABLE checkouts ADD COLUMN due_time TEXT NOT NULL DEFAULT '17:00'"
                )

            reservation_columns = self.connection_manager.table_columns(
                connection,
                "reservations",
            )
            ...
            if "reservation_location" not in reservation_columns:
                connection.execute(
                    "ALTER TABLE reservations ADD COLUMN reservation_location TEXT NOT NULL DEFAULT ''"
                )
    ...
    def add_asset(
        ...
        asset_tag, name, category, location = [
            str(value or "").strip()
            for value in (asset_tag, name, category, location)
        ]

        if not all((asset_tag, name, category, location)):
            raise ValueError(
                "Asset tag, name, category, and location are required."
            )
        ...
        try:
            ...
            raise ValueError(
                "Borrowed quantity must be a non-negative whole number."
            ) from error

        if borrowed_quantity < 0 or borrowed_quantity > quantity:
            raise ValueError(
                "Borrowed quantity cannot exceed total quantity."
            )
        ...
        try:
            with self.connect() as connection:
                connection.execute(
                    "INSERT INTO assets "
                    "(asset_tag, name, category, quantity, borrowed_quantity, location, condition) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (
                        asset_tag,
                        name,
    ...
    def assets(self, search=""):
        ...
        with self.connect() as connection:
            return connection.execute(
                ...
                "a.quantity - a.borrowed_quantity - "
                "COALESCE((SELECT SUM(c.quantity) FROM checkouts c "
                ...
                "AND r.start_date <= ? "
                "AND r.end_date >= ?), 0) <= 0 "
                "THEN 'Out of Stock' ELSE a.status END AS status "
    ...
    def reserve(
        ...
        if not borrower or not purpose:
            raise ValueError(
                "Borrower and purpose are required."
            )
        ...
        with self.connect() as connection:
            ...
            if asset["status"] != "Available":
                raise ValueError(
                    "This asset is not available for reservation."
                )
            ...
            if (
                ...
                raise ValueError(
                    "Not enough available quantity for those dates."
                )
            ...
            cursor = connection.execute(
                ...
                "(asset_id, borrower, quantity, start_date, end_date, "
                "purpose, reservation_location, status) "
                "SELECT ?, ?, ?, ?, ?, ?, location, ? "
                "FROM assets "
                "WHERE id = ? "
                "RETURNING id",
            ...
            self._audit(
                ...
                borrower,
                "RESERVATION_CREATED",
                f"asset_id={asset_id}, status={status}",
            )
    ...
    def approve_reservation(
        ...
        with self.connect() as connection:
            ...
            self._audit(
                ...
                "admin",
                "RESERVATION_UPDATED",
                f"reservation_id={reservation_id}, status={status}",
            )
    ...
    def pending_reservations(self):
        with self.connect() as connection:
            return connection.execute(
                "SELECT r.id, a.asset_tag, a.name, r.borrower, "
                "r.quantity, r.start_date, r.end_date, r.purpose "
                "FROM reservations r "
    ...
    def checkout(
        ...
        checkout_location = str(
            checkout_location or ""
        ).strip()

        if not borrower or not checkout_location:
            raise ValueError(
                "Borrower and checkout location are required."
            )
        ...
        with self.connect() as connection:
            ...
            asset = connection.execute(
                "SELECT quantity, status, location "
                "FROM assets "
                "WHERE id = ?",
                (asset_id,),
            ).fetchone()
            ...
            if not asset or asset["status"] != "Available":
                raise ValueError(
                    "This asset is not available for checkout."
                )

            borrower_record = connection.execute(
                "SELECT role "
                "FROM users "
                "WHERE username = ?",
                (borrower,),
            ...
            if (
                not borrower_record
                or borrower_record["role"] != "STUDENT"
            ):
                raise ValueError(
                    "Only registered students can be assigned as borrowers."
                )

            if asset["location"] != checkout_location:
                raise ValueError(
                    "Checkout location must match the asset's registered location."
                )

            reservation = connection.execute(
                "SELECT DISTINCT reservation_location "
                "FROM reservations "
                "WHERE asset_id = ? "
                "AND status = 'ACTIVE' "
                "AND start_date <= ? "
            ...
            reservation_locations = {
                row["reservation_location"]
                for row in reservation
                if row["reservation_location"]
            }

            if (
                reservation_locations
                and checkout_location not in reservation_locations
            ):
                raise ValueError(
                    "Checkout location does not match the reservation location."
                )
            ...
            if (
                ...
                raise ValueError(
                    "No available quantity remains for checkout."
                )

            connection.execute(
                "INSERT INTO checkouts "
                "(asset_id, borrower, quantity, due_date, "
                "checkout_location, due_time) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (
                    asset_id,
                    borrower,
    ...
    def active_checkouts(self):
        with self.connect() as connection:
            return connection.execute(
                "SELECT c.id, a.asset_tag, a.name, c.borrower, "
                "c.quantity, c.due_date, c.checkout_location, c.due_time, "
                "CASE WHEN a.status = 'Available' "
                "AND a.quantity - a.borrowed_quantity - "
                "COALESCE((SELECT SUM(active.quantity) "
                "FROM checkouts active "
                "WHERE active.asset_id = a.id "
                "AND active.returned_at IS NULL), 0) <= 0 "
                "THEN 'Out of Stock' ELSE a.status END AS status "
                "FROM checkouts c "
    ...
    def registered_students(self):
        with self.connect() as connection:
            table_exists = self.connection_manager.table_exists(
                connection,
                "users",
            )
    ...
    def export_inventory_csv(
        ...
        with self.connect() as connection:
            ...
            if (
                not user
                or user["role"]
                not in {"ADMIN", "TECHNICIAN"}
            ):
                raise PermissionError(
                    "Only management users can export inventory reports."
                )
    ...
    def update_status(
        ...
        with self.connect() as connection:
            ...
            self._audit(
                ...
                f"asset={asset['asset_tag']}, status={status}",
            )
