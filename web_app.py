...
def create_app(database_path=None):
    # Use the explicitly supplied database path first.
    # If none was supplied, use DATABASE_URL from .env / environment variables.
    database_path = database_path or os.getenv("DATABASE_URL")

    # Stop immediately with a clear error instead of producing
    # a confusing psycopg "getaddrinfo failed" error.
    ...
    # Both controllers use the same Supabase PostgreSQL database.
    ...
    def management_user():
        ...
        return (
            user
            if user and user.get("role") in {"ADMIN", "TECHNICIAN"}
            else None
        )
    ...
    def login():
        if request.method == "POST":
            ...
            success, message, user = current_app.config["AUTH"].login(
                username,
                password,
            )
    ...
    def inventory():
        ...
        if (
            ...
            if not management_user():
                flash(
                    "Only administrators and technicians can add assets.",
                    "error",
                )
                return redirect(url_for("inventory"))
        ...
        pending = (
            current_app.config["STORE"].pending_reservations()
            if management_user()
            else []
        )
    ...
    def checkout():
        ...
        if user["role"] not in {"ADMIN", "TECHNICIAN"}:
            flash(
                "Only administrators and technicians can complete checkouts.",
                "error",
            )
            return redirect(url_for("inventory"))
    ...
    def return_checkout(checkout_id):
        try:
            current_app.config["STORE"].return_checkout(
                checkout_id,
                request.form.get("return_condition", "Good"),
            )
    ...
    def approve_reservation(reservation_id):
        if not management_user():
            flash(
                "Only administrators and technicians can approve requests.",
                "error",
            )
            return redirect(url_for("student"))
        ...
        try:
            ...
            current_app.config["STORE"].approve_reservation(
                reservation_id,
                approved=approved,
            )
    ...
    def export_inventory():
        ...
        try:
            report_path = current_app.config["STORE"].export_inventory_csv(
                user["username"]
            )

            flash(
                f"Inventory exported to {report_path}.",
                "success",
            )
